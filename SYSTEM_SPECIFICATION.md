...
**System Specification Document - Version 6.56**
...
1.  **2箇所同時バージョン更新の徹底**:
    コードに変更を加える際は、必ず以下の2箇所を同時に新しい数値（例: `6.56`）にインクリメントして書き換えること。
    *   **各ファイルの冒頭コメント**: `// components/AIAvatar.tsx - v6.56 - 2026-06-30 - ...`
    *   **プログラム内部のバージョン定数**: `VERSION = "6.56"`, `metadata.json` 内の `"version": "6.56"` など。
2.  **一意なコンテキストマッチング**:
