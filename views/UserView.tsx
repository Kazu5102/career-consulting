...
// views/UserView.tsx - v4.61 - Reverted Install Guide
      ...
      <main className={`flex-1 flex flex-col items-center ${view === 'chatting' ? 'p-4 md:p-6 overflow-hidden h-full' : 'p-0 sm:p-4 md:p-6'}`}>
        {view === 'dashboard' ? 
            <UserDashboard 
                ...
                onResume={(c) => { setMessages(c.messages); setAiName(c.aiName); setAiType(c.aiType); setAiAvatarKey(c.aiAvatar); setCurrentConversationId(c.id); setView('chatting'); setOnboardingStep(6); }} 
                userId={userId} 
                nickname={nickname} 
                pin={pin} 
